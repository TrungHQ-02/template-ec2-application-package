...
# access logs: use s3 to store access logs
...
# connetion logs: use s3 to store connection logs, only valid for ALB :)
...
# client keep alive: the maximum length of time that the ALB maintains a connection to a client 
...
# For ALB, CLB only (no extra charge)
...
### concat the tls and the cipher suite into the header
# only for ALB
...
# for alb only
# period of time an existing client or target connection can remain inactive, with no data being sent or received, before the load balancer closes the connection.
...
### IMPORTANT: 1 of 3 type: application, network, gateway
...
# FOR ALB only, preserve the HOST header
...
# Granular control over subnet configuration
...
# FOR ALB only
# Behavior over the client IP to preserve or not
...
# For TF behavior (how long should TF wait for creation, update, deletion)
variable "timeouts" {
